...
export async function apiFetch(path: string, options: RequestInit = {}) {
  ...
  // Only auto-logout on 401 for authenticated routes, not for login itself
...
export const api = {
  ...
  uploadProfileImage: async (
    ...
    const { status, text } = await uploadFormData(
  ...
  uploadEntityImages: async (
    ...
    const { status, text } = await uploadFormData(
      `/entities/${segment}/${entityId}/image`,
      formData,
      { onProgress: options?.onProgress },
    );
