...
export default function ApprovalChainEditor({ entityName, initial, onSave, onCancel }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-primary-border shadow-sm p-6">
      ...
      {error && (
        <div className="mb-4 bg-danger-light border border-danger-border text-danger text-xs px-3 py-2 rounded-lg">{error}</div>
      )}
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${
              bypass ? 'translate-x-6' : 'translate-x-0.5'
            }`} />
          </button>
        </div>
        {bypass && (
          <div className="mt-2 text-xs text-warning bg-warning-light rounded px-2 py-1">
            ...
            {steps.map((step, idx) => (
              <div key={idx}
                ...
                <button onClick={() => removeStep(idx)}
                  className="text-danger hover:text-danger text-sm px-1">✕</button>
              </div>
            ))}
          </div>
        )}
      ...
      {/* Notify channels — read-only preview for now */}
      <div className="mb-5 bg-gray-50 rounded-xl border border-gray-200 p-3 text-xs text-gray-500">
        <span className="font-semibold text-gray-700">Notify channels</span> are configured separately per company.
        Currently: WhatsApp ✓ · Email ✓ · Push ✓ · Call ✗
      </div>
